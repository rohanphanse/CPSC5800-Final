...
OUTPUT_ROOT = REPO_ROOT / "yolo_open_closed_hand"
...
def label_id(dataset, label_value):
    if dataset == "hagridv2":
        # hagrid labels are numeric: fist=1 (closed), palm/stop/stop_inverted=0 (open)
        try:
            lv = int(label_value)
            return 1 if lv == 1 else 0
        except ValueError:
            return 0
    # ASL labeled and RPS labeled use folder names 0/1
    try:
        lv = int(label_value)
        return 1 if lv == 1 else 0
    except ValueError:
...
def write_split(split, grouped):
    ...
    for rel_path, info in tqdm(grouped.items(), desc=f"write {split}", leave=False):
        ...
        with dest_lbl.open("w", encoding="utf-8") as f:
            for bbox, lv in zip(info["bboxes"], info["labels"]):
                x_center, y_center, bw, bh = normalize_bbox(bbox, info["width"], info["height"])
                cls_id = label_id(info["dataset"], lv)
...
def main():
    ...
    write_data_yaml()
    print("Done building open/closed hand dataset.")
